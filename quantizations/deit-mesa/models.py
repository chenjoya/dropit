...
class Attention(nn.Module):
    ...
    def forward(self, x, norm):
        ...
        B, N, C = x.shape
        x = self.qkv(x)
        x = x.reshape(B, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
...
class Block(nn.Module):
    ...
    def forward(self, x):
        x = x + self.drop_path(self.attn(x, self.norm1))
        x = x + self.drop_path(self.mlp(x, self.norm2))
        return x
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
                 ...
                 drop_path_rate=0., hybrid_backbone=None, norm_layer=ms.LayerNorm, pretrained_cfg=None):
        super().__init__()
